...
class _AdminHomeState extends State<AdminHome> {
  ...
  Future<void> fetchEvents() async {
    const String apiUrl = "http://192.168.137.164:3000/api/events"; // Update with your API URL
    ...
    try {
      ...
      if (response.statusCode == 200) {
        List<dynamic> eventData = jsonDecode(response.body);
        setState(() {
          events = eventData.map((eventData) => Event.fromJson(eventData)).toList();
        });
  ...
  Future<void> deleteEvent(String eventId) async {
    final String apiUrl = "http://192.168.137.164:3000/api/events/$eventId";
  ...
  void navigateToUpdateScreen(Event event) {
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => AddEventDetailPage(), // Pass event data
      ),
    ).then((_) => fetchEventData()); // Refresh event list after update
  }
  @override
  Widget build(BuildContext context) {
    return Scaffold(
              ...
              isLoading
                ...
                children: filteredEvents.map((event) {
                  return Card(
                    ...
                    child: Container(
                      padding: EdgeInsets.all(16), // Padding for better spacing
                              ...
                              Row(
                                children: [
                                  IconButton(
                                    ...
                                    onPressed: () => navigateToUpdateScreen(event),
                                  ),
                                  IconButton(
